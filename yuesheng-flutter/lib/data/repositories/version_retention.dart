...
///
/// **两条「已证边界」（登记在案，勿当 bug 顺手修）**：
///   · 桶标签从 **1** 起：`w0` / `d0` / `m0` **不可达**。上级分档用 `<=` 含等号边界
///     吃掉整点（`age == 86400` 落 `h24`、`== 604800` 落 `d7`、`== 2592000` 落 `w4`），
///     故进 m 分支的**必要**条件是 `age > 2592000` ⇒ `age ~/ 2592000 >= 1`。
///     仅 `h0` 可达（h 分支是唯一以 0 起档的）。编号位移是分档后缀的必然结果，
///     不是 bug。
///   · **末档桶宽被截短**：`h24` 仅含 `age = 86400` 单点、`d7` 仅含 `age = 604800`
///     单点、`w4` 实际只覆盖 `age ∈ [2419200, 2592000]`（28d–30d，约 2 天，非 7 天）。
///     效果是**保留更多**（安全方向），无标签碰撞、不破坏单调性；
///     **这是有意保留的现状，不是待修项**。
