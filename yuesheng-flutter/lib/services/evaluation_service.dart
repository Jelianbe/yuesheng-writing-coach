...
extension EvaluationRoundExtension on EvaluationService {
  ...
  >
  _loadRoundHistory(String sessionId) async {
  ...
  /// 逐症候构建明细（R-019 拆出：computeRoundEvaluation）。
  Future<List<SyndromeEvaluationDetail>> _buildSyndromeDetails(
    String sessionId,
    List<Map<String, dynamic>> confirmationRecords,
    List<Map<String, dynamic>> diagnosisRecords,
  ) async {
  ...
  double _resolvePassRate(
    List<SyndromeEvaluationDetail> syndromeDetails,
    int confirmedCount,
    int totalConfirms,
  ) {
    ...
    if (totalConfirms > 0) return confirmedCount / totalConfirms;
    return 0.5;
  }

  /// 趋势：优先从症候明细聚合（R-019 拆出：computeRoundEvaluation）。
  EvaluationTrend _resolveEvaluationTrend(
    List<SyndromeEvaluationDetail> syndromeDetails,
    double passRate,
    List<DiagnosisRow> diagnoses,
  ) {
  ...
  int? _resolveSeverityDelta(int round, List<DiagnosisRow> diagnoses) {
    ...
    return (currSeverity - prevSeverity).round();
  }

  /// 组装评估结果（R-019 二次拆：computeRoundEvaluation）。
  EvaluationData _assembleEvaluationData({
    required int round,
    required EvaluationTrend trend,
...
/// 装载本轮评估历史（R-019 拆出：computeRoundEvaluation）。
