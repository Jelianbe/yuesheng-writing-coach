...
class TaskPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (problems.isEmpty) return _buildEmptyState(context);
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (onSelfPractice != null) _buildSelfPracticeFooter(context),
        ],
      ),
    );
  }
  ...
  Widget _buildSelfPracticeFooter(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.symmetric(
        horizontal: AppSpacing.lg,
      ...
      child: Align(
        ...
        child: TextButton.icon(
          ...
          label: const Text('换个问题练？自选练习', style: TextStyle(fontSize: 13)),
  ...
  Widget _buildProblemRow(BuildContext context, ActiveProblemView problem) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          _buildProblemMarker(context, problem, severity),
          const Spacer(),
          if (onMarkComplete != null) _buildCompleteBtn(context, problem),
          if (onRemove != null) ..._buildRemoveBtn(context, problem),
        ],
      ),
    );
  }

  Widget _buildProblemMarker(
    BuildContext context,
    ActiveProblemView problem,
    ({Color text, Color bg}) severity,
  ) {
    return Container(
  ...
  Widget _buildCompleteBtn(BuildContext context, ActiveProblemView problem) {
    return InkWell(
