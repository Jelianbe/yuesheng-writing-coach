...
void main() {
  ...
  final allowedIds = kAllSyndromeIds
      .toSet()
  ...
  group('批次3（3.2）症候库基础', () {
    test('#1 权威集合由注册表派生（活跃共 ${kSyndromeIds.length} 个）', () {
      ...
      for (int i = 0; i < kSyndromeRegistry.length; i++) {
        expect(kSyndromeRegistry[i].id, 'P${(3 + i).toString().padLeft(3, '0')}',
    ...
    test('#2 症候库自洽：索引表/手册有条目，L3 检索非空', () {
      for (final id in validSyndromes) {
        expect(kSyndromeIndexContent, contains('| $id '),
            reason: '症候索引表缺 $id');
        expect(kSyndromeManualContent, contains('### $id '),
            reason: '症候手册缺 $id');
        expect(getSyndromeContent([id]), isNotEmpty,
            reason: 'L3 检索 $id 为空');
      }
    });
    ...
    test('#12 症候库表格行由注册表渲染（双向逐字一致，b9 批次28）', () {
      ...
      for (final s in kSyndromeRegistry.where((s) => s.retired != true)) {
        ...
        expect(kSyndromeIndexContent, contains(indexRow),
            reason: '索引表渲染行缺失 ${s.id}: $indexRow');

        final typeRow =
            '| ${s.type.value} | ${s.id} ${s.v1ActionDisplayName} | ${s.typeLine} |';
        expect(kSyndromeManualContent, contains(typeRow),
            reason: '类型速查渲染行缺失 ${s.id}: $typeRow');

        final t = s.techniques.first;
        final techRow = '| ${s.id} ${s.shortName} | $t ${techniqueNameOf(t) ?? ''} |';
        expect(kSyndromeManualContent, contains(techRow),
            reason: '技法映射渲染行缺失 ${s.id}: $techRow');
      }

      // 反向：内容 → 注册表（表格行数与注册表一致，防多余/重复行）
      final indexRows = RegExp(r'^\| (P\d{3}) \| ', multiLine: true)
          .allMatches(kSyndromeIndexContent)
          .map((m) => m.group(1)!)
          .toList();
      expect(indexRows, kSyndromeIds,
          reason: '索引表行数与注册表不一致');

      final typeRows = RegExp(r'^\| [a-z_]+ \| (P\d{3}) ', multiLine: true)
          .allMatches(kSyndromeManualContent)
          .map((m) => m.group(1)!)
          .toList();
      expect(typeRows.length, kSyndromeRegistry.length,
          reason: '类型速查表行数与注册表不一致');

      final techRows = RegExp(r'^\| (P\d{3}) [^|]+ \| T\d{3} ', multiLine: true)
          .allMatches(kSyndromeManualContent)
          .map((m) => m.group(1)!)
          .toList();
      expect(techRows, kSyndromeIds,
          reason: '技法映射表行数与注册表不一致');
      ...
      final l2Rows = RegExp(r'^\| (P\d{3}) \| T\d{3} ', multiLine: true)
          .allMatches(kTechniqueIndexContent)
          .map((m) => m.group(1)!)
          .toList();
      expect(l2Rows, kSyndromeIds,
      ...
      final v2Rows = RegExp(r'^\| (P\d{3}) ', multiLine: true)
          ...
          .map((m) => m.group(1)!)
          .toList();
      expect(v2Rows, kSyndromeIds,
          reason: 'v2 动作映射行数与注册表不一致');
      final tIndexRows = RegExp(r'^\| (P\d{3}) \| ', multiLine: true)
          .allMatches(templatesIndex)
          .map((m) => m.group(1)!)
          .toList();
      expect(tIndexRows, kSyndromeIds,
  ...
  group('批次3（3.2）训练库一致性', () {
    test('#3 训练库每症候非空（活跃 ${kSyndromeIds.length}）', () {
      for (final id in validSyndromes) {
        expect(kTrainingFullKnowledge, contains('## $id '),
            reason: '训练知识缺 $id');
        expect(getTrainingContent([id]), isNotEmpty,
            reason: '训练检索 $id 为空');
      }
    });
  });
  ...
  group('批次3（3.2）技法库双向存在', () {
    test('#4 映射键均为合法症候，技法 ID 在技法库有完整条目', () {
      for (final entry in kTechniquesBySyndrome.entries) {
        expect(validSyndromes.contains(entry.key), true,
    ...
    test('#5 技法库覆盖全部 ${kSyndromeRegistry.length} 症候（双向存在）', () {
      for (final id in validSyndromes) {
        expect(kTechniquesBySyndrome.containsKey(id), true,
            reason: '症候 $id 在技法映射中缺失');
      }
    });

    test('#6 技法索引表引用的技法 ID 在完整库存在', () {
      final indexTechniques = RegExp(r'\| T(\d{3}) ')
          .allMatches(kTechniqueIndexContent)
          .map((m) => 'T${m.group(1)}')
          .toSet();
  ...
  group('批次3（3.2）动作库一致性', () {
    test('#7 动作映射表首列症候合法，动作编号均有条目', () {
      for (final skillId in ['coaching-actions', 'coaching-actions-v2']) {
        ...
        // 映射表首列症候 ID
        final syndromeRefs = RegExp(r'^\| (P\d{3}) ', multiLine: true)
            .allMatches(content)
            .map((m) => m.group(1)!)
            .toSet();
        expect(syndromeRefs, isNotEmpty, reason: '$skillId 无症候映射表');
        for (final id in syndromeRefs) {
          expect(allowedIds.contains(id), true,
              reason: '$skillId 映射表引用非法/退役症候 $id');
        }

        // 动作编号条目标题均存在
        final actionIds = RegExp(r'^### (A\d{3}) ', multiLine: true)
            .allMatches(content)
            .map((m) => m.group(1)!)
            .toSet();
        ...
        final actionRefs = RegExp(r'\bA\d{3}\b')
            .allMatches(content)
            .map((m) => m.group(0)!)
            .toSet();
        for (final a in actionRefs) {
          expect(actionIds.contains(a), true,
              reason: '$skillId 引用未定义动作 $a');
        }
      }
    });
  });
  ...
  group('批次3（3.2）互斥对与内容引用', () {
    test('#8 互斥对引用 ID 存在（P006/P021、P015/P012、P009/P018）', () {
      ...
      expect(manual, contains('排除 P021'),
          reason: 'P006 互斥校验（排除 P021）缺失');
      expect(manual, contains('排除 P006'),
          reason: 'P021 互斥校验（排除 P006）缺失');
      expect(manual, contains('P012 张力不足'),
          reason: 'P015 铺垫质量前置（P012）缺失');
      expect(manual, contains('区分 P018'),
          reason: 'P009/P018 触发信号差异化缺失');
      expect(manual, contains('P028 优先'),
          reason: 'P028/P003 重叠优先级（P028 优先）缺失');
      expect(manual, contains('P006 优先'),
          reason: 'P029/P006 重叠优先级（P006 优先）缺失');
      expect(manual, contains('P030 优先'),
          reason: 'P030/P006 重叠优先级（P030 优先）缺失');
      expect(manual, contains('P031 设定矛盾 vs P018 人设崩塌'),
          reason: 'P031/P018 重叠优先级（P018 优先）缺失');
      expect(manual, contains('P032 金手指失衡 vs P012 张力不足'),
          reason: 'P032/P012 重叠优先级（P032 优先）缺失');
      expect(manual, contains('P033 升级节奏失衡 vs P030 节奏比例失衡'),
          reason: 'P033/P030 重叠优先级（P030 优先）缺失');
      expect(manual, contains('P034 配角工具人 vs P009 角色空心化'),
          reason: 'P034/P009 重叠优先级（P009 优先）缺失');
      expect(manual, contains('P035 对话注水 vs P011 对话疲劳'),
          reason: 'P035/P011 重叠优先级（P011 优先）缺失');
      expect(manual, contains('P036 流水账叙述 vs P006 节奏停滞'),
          reason: 'P036/P006 重叠优先级（P006 优先）缺失');
      expect(manual, contains('P036 流水账叙述 vs P021 跳跃叙事'),
          reason: 'P036/P021 重叠优先级（P021 优先）缺失');
      expect(manual, contains('P039 目标模糊 vs P040 被动主角'),
          reason: 'P039/P040 重叠优先级（P039 优先）缺失');
      expect(manual, contains('P040 被动主角 vs P016 情节巧合过多'),
          reason: 'P040/P016 重叠优先级（P016 优先）缺失');
      expect(manual, contains('P041 降智反派 vs P012 张力不足'),
    ...
    test('#9 内容引用漂移：全库出现的症候 ID 均合法（防 P023+ 悬空引用）', () {
      ...
      final foundIds = RegExp(r'\bP0(0\d|1\d|2\d|3\d|4\d)\b')
          .allMatches(allContent)
          .map((m) => m.group(0)!)
          .toSet();
      final illegal = foundIds.difference(allowedIds);
      expect(illegal, isEmpty,
          reason: '内容引用非法/悬空症候 ID: $illegal');
    });
  });
  ...
  group('批次3（3.1）token 预算静态表', () {
    test('#10 最坏情形合计超模型上下文（需降级），warning 线也超', () {
      ...
      final warning = (TokenEstimate.maxBudget * TokenEstimate.warningRatio)
          .round();
      expect(total, greaterThan(warning),
          reason: '最坏情形 $total 应超过 warning 线 $warning（说明必须启用降级）');
      expect(total, greaterThan(TokenEstimate.maxBudget),
          reason: '最坏情形 $total 应超过 maxBudget ${TokenEstimate.maxBudget}');
    });

    test('#11 溢出降级顺序：L2 组 → L3 非 focus → 检测器 → … → 历史最后', () {
      final order = TokenBudgetTable.planDegradation().map((s) => s.name).toList();
      expect(order.length, 8);
      expect(order[0], contains('L2 按需组'),
          reason: '最先裁 L2 组数');
      expect(order[1], contains('L3 结构化'),
          reason: '其次裁 L3 非 focus 概览');
      expect(order[2], contains('规则观察检测器'),
          reason: '再次裁检测器注入');
      expect(order.last, contains('历史消息'),
          reason: '历史消息最后裁（保底前的最后手段）');
    });

    test('#12 临场输出约束与协议块为保底层（永不裁）', () {
      final bottom = TokenBudgetTable.bottomLineStages;
      expect(bottom.any((s) => s.contains('临场输出约束')), true,
          reason: '临场输出约束必须保底');
      expect(bottom.any((s) => s.contains('[YS_ENTITY]')), true,
          reason: '协议块说明必须保底（不注入 AI 就不输出）');
      expect(bottom.any((s) => s.contains('L1 核心 skill')), true,
