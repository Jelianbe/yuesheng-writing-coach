...
class _MigrationTriggerLlmClient extends LlmClient {
  ...
  Future<void> streamChat(
    ...
    final body = '诊断完成。本轮复诊：该症候仍为当前教学焦点，建议继续巩固。\n'
        '[YS_DIAGNOSIS]\n'
        ...
        '[/YS_DIAGNOSIS]';
    callback(
      LlmStreamResponse(content: body, isDone: false),
    );
  ...
  test(
    ...
    () async {
      ...
      try {
        ...
        final sedimentTotal =
            entities.length + charFacts.length + eventFacts.length +
            subplotFacts.length;
        // ignore: avoid_print
        print(
        ...
        for (final p in problemsAfterEval) {
          ...
          expect(
            ts == null ||
                const ['identified', 'in_progress', 'consolidating', 'mastered']
                    .contains(ts),
            isTrue,
        ...
        await studentModelRepo.appendTeachingHistory(sessionId, {
          'type': 'confirmation',
          'syndromes': [problemsForMigration.isNotEmpty
