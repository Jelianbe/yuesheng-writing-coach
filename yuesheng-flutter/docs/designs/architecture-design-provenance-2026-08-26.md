...
- **UI 样式族令牌化进展**（AppTextStyles / AppBoxStyles / AppButtonStyles 三族，独立于 P0-2 的 AppRadius/AppSpacing）：
  ...
  - **Batch6（2026-08-27, commit d0756d8a+3d463b2c）**：writing_coach_panel_teaching (titleLg+body) + placeholder_page (titleLg+body) + onboarding_questionnaire (titleLg×2+body) + session_drawer (titleLg+body) + diagnosis_picker_sheet (titleLg) + search_replace_sheet (caption) 共 10 处替换，全部匹配现有令牌无新增（R-019 防通胀）。原计划 14 处，实际：diagnosis_picker_sheet 与 search_replace_sheet 剩余 4 处为非标值（fontSize 13/15/16 + w700、不同语义颜色），按 R-019 保留硬编码。无逻辑变化，无新增 import。四道门禁：analyze 0 issues (2.0s) / widgets 784 全绿 / 0 循环依赖（286 模块） / 0 密钥命中
  - **Batch7（2026-08-27, commit e374e338）**：chat_page (body) + chat_teaching (titleLg+body) + message_list (titleLg+body) + evaluation_report_panel (caption×2) + observation_audit_card (caption) 共 8 处替换。其中 chat_teaching 与 message_list 的 titleLg 原为多行格式 TextStyle 字面量，单行正则未匹配，本批顺带补齐。无新增令牌、无逻辑变化、无新增 import。四道门禁：analyze 0 issues (2.0s) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **Batch8（2026-08-27, commit 0d62ebaf）**：excerpt_picker_sheet (caption) + growth_detail_page (caption+body) + growth_page (body) + gen_ui_card (caption×3) + project_settings_page (caption) + recycle_bin_sheet (caption) + quick_phrase_sheet (caption) + syndrome_detail_modal (caption) + writing_stats_sheet (caption×2) + writing_curve_chart (caption) 共 12 处替换。无新增令牌、无逻辑变化、无新增 import。本批后全 lib/ 已无符合现有令牌的硬编码 TextStyle（含多行格式），widgets/ AppTextStyles 令牌化收尾。四道门禁：analyze 0 issues (2.1s) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **subCaption 令牌新增（2026-08-27, commit 53b6254e）**：ROI 评估后单独新增 1 个令牌（fontSize:13 + textTertiary，比 caption 大一档）。扫全 lib/ 非标值：formTitle(16/w600) 0 处不新增，subCaption 15 处分布在 15 个文件，频次断层最高 + 语义高度一致（操作说明/辅助提示）。替换 15 处：append_chapters(2) + chapter_recycle_bin + diagnosis_picker + editor_settings + project_settings(2) + quick_phrase + recycle_bin + reference_picker + related_sessions + session_drawer + style_profile + task_panel + version_time_machine。AppTextStyles 5→6 个。四道门禁：analyze 0 issues (1.9s) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **AppButtonStyles.secondary 修正 + 推广（2026-08-27, commit 82881dfb）**：ROI 评估发现 Batch3 三族令牌（standardInput/primary/secondary）凭空构造无设计稿依据，且 Batch3 替换 bookshelf_page 第 820 行时丢失原 padding(lg, md) 引入视觉回归（secondary 错误定义为 padding(vertical md only)）。修正 secondary 定义为 padding(lg, md)，恢复 bookshelf_page 原视觉（自动修复回归）。推广 3 处 AlertDialog 取消按钮到 secondary：session_drawer + message_list + writing_coach_panel_teaching（之前都是 padding(lg, md) + borderRadius md + surface 完全一致）。primary 重新评估不推广（3 处 ElevatedButton 场景各异，padding/borderRadius/disabled 配置都不同，新增 disabledPrimary 变体复用价值低）。standardInput 不推广（25 处 InputDecoration 用 9 种变体，无真实高频模式）。四道门禁：analyze 0 issues (2.0s) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **subBody 令牌新增（2026-08-27, commit d99956ca）**：重新评估非标值发现台账原估严重低估（原估 6 处，实际 39 处分布在 24 文件）。multiline grep `fontSize: 13,[^)]*?color: AppColors.textSecondary` 精确统计后 subBody 频次远超 subCaption 新增时 15 处。纯形 24 处直接替换为 `AppTextStyles.subBody`，变体 15 处用 `.copyWith(fontWeight/height)`（editor_settings_sheet 6 处 w600 变体最集中，需去 const）。main.dart bodySmall 定义也纳入。AppTextStyles 6→7 个。四道门禁：analyze 0 issues (lib) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **noteCaption 令牌新增（2026-08-27, commit a7b6ec5f）**：台账原估 10 处低估，multiline grep `fontSize: 12,[^)]*?color: AppColors.textSecondary` 实际 23 处分布在 15 文件。纯形 19 处直接替换为 `AppTextStyles.noteCaption`，变体 4 处用 `.copyWith(height/fontWeight)`（bookshelf_page + growth_detail_page + search_replace_sheet 各 1 处 height 变体，writing_menu_sheet 1 处 w600 变体）。gen_ui_card 单文件 4 处最集中。AppTextStyles 7→8 个。四道门禁：analyze 0 issues (lib) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **microCaption 令牌新增（2026-08-27, commit b36ac753）**：台账原估 5 处低估，multiline grep `fontSize: 11,[^)]*?color: AppColors.textTertiary` 实际 23 处分布在 18 文件。100% 纯形无变体，全部直接替换为 `AppTextStyles.microCaption`。AppTextStyles 8→9 个。size×color 网格关键补位完成（11/tertiary 为最小号文字）。无新增 import、无逻辑变化。四道门禁：analyze 0 issues (lib) / widgets 784 全绿 / 0 循环依赖 (286 模块) / 0 密钥命中
  - **累计替换**：≥204 处 TextStyle 字面量 → AppTextStyles 令牌（Batch1-8 + subCaption + subBody + noteCaption + microCaption 总和）
  - **非标值保留**（R-019 防通胀）：三批重新评估后全部新增（subBody 39 + noteCaption 23 + microCaption 23 = 85 处），台账原估 21 处严重低估 4 倍。AppTextStyles 5→9 个，size×color 网格补位完成
  - **下次候选**：AppTextStyles 九族令牌已覆盖全 lib/ 高频断层（11-18px × tertiary/secondary 色系）。AppBoxStyles/AppButtonStyles 推广评估已收尾（secondary 修正+推广 3 处，primary/standardInput 不推广）。三族令牌化全面收尾，后续按实际改色/改字号需求再评估扩张
...
| **T-04** | **mes_example few-shot** | 角色卡用 `<START>` 分隔的好/坏对话示例教 LLM 如何说话 | 无对话示例机制 | ✅ 已落地（2026-08-27，首批 + 第二批 + 第三批 + 第四批 + 第五批 + 第六批 + 第七批 + 第八批共 29 个高频症候，全覆盖收官 100%） | 在"教"环节引入 few-shot 示例块（好/坏写作片段对比），按症候类型动态组装。新建 `training_few_shot_library.dart` 独立模块，首批 5 个（P003/P004/P008/P011/P019）+ 第二批 5 个（P005/P006/P007/P009/P010）+ 第三批 3 个（P012 张力不足/P013 开篇平庸/P014 结尾乏力症）+ 第四批 3 个（P015 高潮疲软/P016 巧合过多/P017 伏笔失效）+ 第五批 3 个（P018 人设崩塌/P020 过渡生硬/P021 跳跃叙事）+ 第六批 3 个（P022 重复用词/P023 爽点乏力/P024 期待感断裂）+ 第七批 3 个（P025 黄金三章失效/P026 章节钩子缺失/P027 追读动力不足）+ 第八批 4 个（P028 画面感缺失/P029 段落失控/P030 节奏比例失衡/P031 设定矛盾），共覆盖 29 个高频症候（全覆盖收官）；`getTrainingFewShot` 在训练知识注入后追加，复用 `trainingKnowledge` 阶段标记（不新增 BudgetStage，零预算表改动） |
