...
- **UI 样式族令牌化进展**（AppTextStyles / AppBoxStyles / AppButtonStyles 三族，独立于 P0-2 的 AppRadius/AppSpacing）：
  ...
  - **累计替换**：≥119 处 TextStyle 字面量 → AppTextStyles 令牌（Batch1-8 + subCaption 总和）
  - **非标值保留**（R-019 防通胀）：noteCaption(12/secondary) 10 处、subBody(13/secondary) 6 处、microCaption(11/tertiary) 5 处等中低 ROI 候选暂不新增，留作后续根据实际改色需求再决定
  - **下次候选**：AppTextStyles 已覆盖全 lib/ 高频断层。AppBoxStyles/AppButtonStyles 推广评估已收尾（secondary 修正+推广 3 处，primary/standardInput 因场景各异不推广，见上行），三族令牌化暂告段落。后续可重新评估 noteCaption/subBody 等 21 处非标值是否值得扩张
