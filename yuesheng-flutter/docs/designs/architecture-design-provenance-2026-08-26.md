...
| **T-04** | **mes_example few-shot** | 角色卡用 `<START>` 分隔的好/坏对话示例教 LLM 如何说话 | 无对话示例机制 | ✅ 已落地（2026-08-27，首批 + 第二批共 10 个高频症候） | 在"教"环节引入 few-shot 示例块（好/坏写作片段对比），按症候类型动态组装。新建 `training_few_shot_library.dart` 独立模块，首批 5 个（P003/P004/P008/P011/P019）+ 第二批 5 个（P005/P006/P007/P009/P010），共覆盖 10 个高频症候；`getTrainingFewShot` 在训练知识注入后追加，复用 `trainingKnowledge` 阶段标记（不新增 BudgetStage，零预算表改动） |
