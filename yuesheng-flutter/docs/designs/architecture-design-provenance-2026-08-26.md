...
- **UI 样式族令牌化进展**（AppTextStyles / AppBoxStyles / AppButtonStyles 三族，独立于 P0-2 的 AppRadius/AppSpacing）：
  ...
  - **累计替换**：≥104 处 TextStyle 字面量 → AppTextStyles 令牌（Batch1-8 总和）
  - **非标值保留**（R-019 防通胀）：fontSize 13/15/16/11 + w700 等组合不强行新增令牌，保留硬编码
  - **下次候选**：全 lib/ 符合现有令牌的硬编码 TextStyle 已扫清（含多行格式）。后续如需继续可评估新增 formTitle/numericCaption 等小家族令牌的 ROI，或将 AppBoxStyles/AppButtonStyles 推广到剩余 InputDecoration/ElevatedButton 字面量
