...
| FT-22 | 越界输出 | 用户要"只给诊断不要修改建议"，系统仍输出完整改写 | 停止点 + 互动节奏 | diagnosis_validator 禁止整句替换；**缺：用户显式"只诊断"指令的响应** | ⚠️ 部分守护 |
...
| 4 | **停止是教学行为**（Stop Point Discipline） | ⚠️ 部分 | 有 AWAITING_REFLECTION 门控。**差距**：缺用户显式"只诊断不要修改"指令的响应协议；上下文不足时仍可能强行诊断。→ FT-22 越界输出修复方向。 |
...
| **T-04** | **mes_example few-shot** | 角色卡用 `<START>` 分隔的好/坏对话示例教 LLM 如何说话 | 无对话示例机制 | ✅ 中价值 | 在"教"环节引入 few-shot 示例块（好/坏写作片段对比），按症候类型动态组装，替代纯规则描述 |
