...
- **UI 样式族令牌化进展**（AppTextStyles / AppBoxStyles / AppButtonStyles 三族，独立于 P0-2 的 AppRadius/AppSpacing）：
  ...
  - **下次候选**：AppTextStyles 已覆盖全 lib/ 高频断层。后续可评估 AppBoxStyles/AppButtonStyles 推广到剩余 InputDecoration/ElevatedButton 字面量，或重新评估 noteCaption/subBody 等 21 处非标值是否值得扩张
