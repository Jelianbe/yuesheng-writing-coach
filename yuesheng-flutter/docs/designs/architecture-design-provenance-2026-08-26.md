...
- **UI 样式族令牌化进展**（AppTextStyles / AppBoxStyles / AppButtonStyles 三族，独立于 P0-2 的 AppRadius/AppSpacing）：
  ...
  - **累计替换**：≥158 处 TextStyle 字面量 → AppTextStyles 令牌（Batch1-8 + subCaption + subBody 总和）
  - **非标值保留**（R-019 防通胀）：台账原估 noteCaption 10/microCaption 5 严重低估，multiline grep 实际 noteCaption(12/secondary) 23 处、microCaption(11/tertiary) 23 处。两者均达高频阈值，计划按 ROI 逐个新增（noteCaption 次批、microCaption 末批）
  - **下次候选**：subBody 已落地（39 处）。剩余 noteCaption(23 处) + microCaption(23 处) 待逐个新增。AppBoxStyles/AppButtonStyles 推广评估已收尾（secondary 修正+推广 3 处，primary/standardInput 不推广）。三族令牌化持续推进中
