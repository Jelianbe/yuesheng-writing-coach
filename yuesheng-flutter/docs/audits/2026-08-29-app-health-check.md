...
> ✅ **`feedback-cognition` 与 `coaching-rhythm §五`「重叠」的核查结论（2026-08-29）**
>
> 清单 §4.3 标注的「内容重叠、会重复注入」经核查**基本不成立**：
> `skills_beginner_p4.dart` §六（L156-180）早已写死分工——本站 §五 Layer 2
> 管**训练前**的桥接（方向/目标/原因）+ 多轮总结，feedback-cognition 管
> **训练后**的对比，两者「共同构成完整的 Layer 2」。一前一后，并非重复。
>
> 真正的瑕疵只有一处：§5.3 为保持格式完整，也列出了「训练完成后的桥接」
> 示例句式，而 §六 的归属说明**遗漏了这一项**，留下歧义空间。
>
> **处理**：在 §六 补 4 行归属澄清（该示例仅作文风示范，实际反馈以
> feedback-cognition 为准，本处不重复触发）。未删改任何既有内容，
> `feedback-cognition` 所在文件一字未动。
>
> **漂移**：锚点 17 处，全部 +132 字节、单一源头 `coaching-rhythm`
> （7676→7808），传导至 5 个注入该 skill 的 prompt。经舰长确认后更新基线。

| 项 | 现状 | 状态 |
|:--|:--|:--|
| 知识注入驱动模型 | Phase 3 已收口：A 组两块落地（advanced-phases / coaching-rhythm），B 组四块不索引化。已 ADR 化 | ✅ 已决 |
| `feedback-cognition` 与 `coaching-rhythm §五` 内容重叠 | 清单 §4.3 注：两者存在重复，需明确分工避免重复注入 | ✅ **已澄清（2026-08-29）**——详见下方核查结论 |
