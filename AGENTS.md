...
（V4.20：**「测试全绿」不等于「覆盖存在」——零直接引用的函数，间接调用往往是虚覆盖**
——2026-09-04 R-019 P1 第五批实证。`student_profile` 家族三个函数在 test/ 里
**零直接引用**，只有经 `buildStudentContext` 的间接覆盖，而 C71 的间接测试全绿。
看着很安全，于是做了变异验证：**6 个变异全部漏网**（早退阈值、两个 `>=` 边界、
样本门槛、效果覆盖判据各一处）。也就是说，这些分支**从来没有被任何断言验证过**，
全绿只是因为它们没被走到——此时重构等于**在没有护栏的路上换轮胎**。
（这是 V4.6「抽测文件名假覆盖」的更深一层：V4.6 是文件名骗你，V4.20 是
「调用链上存在」骗你。函数被调用于是覆盖率好看，但没有任何断言锚定它的判据。）
正确做法：对**任何零直接引用**的函数做重构前，先跑一遍变异验证；漏网就先补
针对判据边界的直接单测（本批补 15 例），复验全部拦截后再动结构。
**判别：动手前问「改坏这一行，哪个断言会红」，答不上来就是虚覆盖。**）
（V4.21：**单点评据只证明「那一条」被锚定，不证明整个函数是真覆盖**
——2026-09-04 批次 F 实证。V4.20 落地时做了一轮安全区摸底，对 8 个函数各打
**1 个**判据级变异，`interventionLevelForTrainingCount` 与 `validateGenuiComponent`
都被判为「真空」——据此决定直接重构。动手前改打**分支级**判据（每个分支各一个，
两函数共 20 个），结果 **12 真 / 8 虚**——40% 分支从未被断言锚定：G2 的
passRate 上界、G3 的连败下界、G5 的 totalCount 下界，以及 stat 缺 label / 缺
max、progress 缺 label、timeline 缺 date、quiz answer 类型。漏掉是因为测试只覆盖
「每类一个字段」——stat 只测了缺 value、timeline 只测了缺 title。摸底只打一个点，
就会把三分之一当成全部。
（这是 V4.20 的更细一层：V4.20 判的是**函数级**有无覆盖，V4.21 判的是**分支级**
覆盖是否完整。一个函数可以「有覆盖」却只覆盖了三分之一分支。）
正确做法：**重构准入按分支数而非函数数计**——对目标函数的每个条件分支各准备一个
判据级变异，全拦截才叫真覆盖；漏网的先补边界用例（本批补 8 例），复验 20/20
再动结构。补测后复验 → 重构后复验（**重构会改变代码形态，原锚点文本可能失效**，
本次 20 个里有 4 个因三元改 if / return null 改 bool 而失配，故脚本里加了行尾自适应
+ 锚点缺失显式报错，**重构后必须同步更新锚点再复验**，否则复验会退化成
「锚点缺失」的假通过）。
另注 V4.18 的门禁脚本层补充：**`scripts/gate.sh` 跑 `flutter test` 时也必须清空
...
（V4.25：**共享工作树跑全量测试，前后必须钉 HEAD 对拍**——2026-09-06 C78 批次2a
实证。同一工作树上有并行会话时，你的未提交改动会被**无声叠加到对方新推的 base**
上（git 只在 commit 时读 HEAD，base 动了不会通知你）：

```
跑之前：git rev-parse HEAD → bfd1a9d9
（2 分半后）
跑之后：git rev-parse HEAD → 2737d292   ← 对方在你跑测试期间推了提交
```

后果：**你无法再用自己的全量测试证明「本批零回归」**——红灯里混着对方的回归。
本次形态：首跑（base=bfd1a9d9）挂 A，第二跑（base=2737d292）A 变绿、冒出 B。
**A 归我、B 归他**，只看两次结果根本切不开。

**标准取证流程（本次的答案）——对拍两个版本的被判定对象**：
```bash
git show bfd1a9d9:<前缀>lib/xxx.dart  | grep -c <关键词>   # 1 → 期望 1，绿
git show 2737d292:<前缀>lib/xxx.dart  | grep -c <关键词>   # 2 → 期望 1，红
...
同批另一条（**与 a 同源：都是头约定与既有机制的交互事故**）：**内容语义计数
必须锚定生效位置，不得整文件计数**（V4.13「注释不是代码」的延伸）。元数据头约定
（L1 常驻补头）落地时未审计存量的「整文件计数类」测试——`syndrome_count_scope_test`
用 `_count(src, needle)` 数**整个源文件**，头部补一句登记文案（`//` 开头、不进
prompt、模型根本看不到）就把「应出现 1 次」顶成 2 次而红。

**判据的介质**：注释里写一百遍也不算数。修法是剥离注释行后计数——只剥「跳过前导
空白后以 `//` 开头」的**整行**，**不剥行内尾注释**（这些文件的行内 `//` 多为字符串
或代码尾注，粗暴剥离会切掉注入正文，制造比原问题更隐蔽的假绿）。改前应先离线核验
剥离对每个判据的影响，确认零误伤。

**改统计口径必须做负向验证**——否则分不清「改对了」和「把护栏改哑」：临时改掉正文
那处合法出现 → 测试**必须红** → 还原。本次实测：改后红、其余 10 项仍绿，还原后
11 项全绿，证明护栏仍精确锚定在正文处。）
