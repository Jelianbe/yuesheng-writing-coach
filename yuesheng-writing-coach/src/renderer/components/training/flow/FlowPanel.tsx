 ...
 *       与原 StepIndicatorList 不冲突，按 flowType 切换。
 *
 * Sprint 25 BL-01 C-3: 迁移自 components_archived/training/flow/FiveStepFlow.tsx
 * 改造: ① 重命名为 FlowPanel（对齐计划 DoD-1）
 *       ② BEM 全局类名 → CSS Modules styles.xxx (R-019)
 *       ③ 类型 import 切到 src/renderer/shared/types-training.ts
 *       ④ 子组件 import 指向同目录 V6.2 组件
 */
...
export function FlowPanel({ active, flow, evaluation, onExit }: Props) {
  ...
  return (
    <section className={styles.fiveStepFlow} data-testid="five-step-flow">
      <header className={styles.fiveStepFlowHeader}>
      ...
      <div className={styles.fiveStepFlowBody}>
      ...
      <footer className={styles.fiveStepFlowFooter}>
        <button
          type="button"
          onClick={goPrev}
