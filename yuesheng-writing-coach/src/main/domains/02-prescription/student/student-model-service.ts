...
export class StudentModelService {
  ...
  // ============================================================
  // 画像持久化层(RWR-P1-7 / C-1)
  // 依据 spec §4.4:teachingHistory[] + attitudePreference 跨 session
  // 约束:
  //   - R-021 隐性诊断:teaching_history 系统内部,前端 UI 永不渲染
  //   - R-020 循环依赖:本段不引用 diagnosis-processor / dispute-tracker
  //   - 本段是"工具",C-3 训练反馈回路时再触发写入
  // 债务标记:本 service 已超 500 行,RWR-P3-2 全局清理时拆 RowManager
  // ============================================================
   ...
   */
  private ensureSessionRow(sessionId: string): string {
