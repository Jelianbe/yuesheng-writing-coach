...
export class AbilityProfileService {
  ...
  computeProfile(sessionId: string): AbilityProfile {
    ...
    const diagnoses = this.diagnosisService.getBySession(sessionId);
    const trainings = this.trainingService.getBySession(sessionId);
    const occurrences = this.aggregateSyndromeOccurrences(diagnoses);

    return {
      sessionId,
      abilities: this.computeAbilityScores(atlas, occurrences),
      weakPoints: this.computeWeakPoints(atlas, occurrences),
      trainingStats: this.computeTrainingStats(trainings),
      diagnosisTrend: this.computeDiagnosisTrend(diagnoses),
