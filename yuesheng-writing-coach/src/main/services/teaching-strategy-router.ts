 ...
 *
 * - teaching-strategy-router.types.ts      内部类型定义
 * - teaching-strategy-router.constants.ts  常量映射
 * - teaching-strategy-router.conditions.ts 条件匹配引擎与工具函数
 * - teaching-strategy-router.layer1.ts     Layer 1：聚焦症候选择
 * - teaching-strategy-router.layer2.ts     Layer 2：教学方式选择
 * - teaching-strategy-router.layer3.ts     Layer 3：参数细化 + 后向兼容桥接
 *
 * 本文件保留为薄类，负责：配置加载（惰性缓存） + decide() 编排。
 * 所有外部导入路径保持不变。
 *
 * @see teaching-strategy-router.layer1
 * @see teaching-strategy-router.layer2
 * @see teaching-strategy-router.layer3
 */
...
import type { RouterConfigs, LearningPathConfig } from './teaching-strategy-router.types';
import { selectFocusSyndrome } from './teaching-strategy-router.layer1';
import { selectTeachingMode } from './teaching-strategy-router.layer2';
import { refineParameters, buildLegacyBridge } from './teaching-strategy-router.layer3';
