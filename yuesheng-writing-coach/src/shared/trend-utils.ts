 ...
 */
export function calcTrendFromAverages(
  currentAvg: number,
  previousAvg: number,
): TrendDirection {
  if (currentAvg < previousAvg * TREND_IMPROVE_THRESHOLD) return 'up';
  if (currentAvg > previousAvg * TREND_WORSEN_THRESHOLD) return 'down';
  return 'stable';
}
 ...
 */
export function calcTrendFromHistory(history: number[]): TrendDirection {
...
export function mapTrendLabel(
  ...
  if (direction === 'down') return 'worsening';
  return 'stable';
}
